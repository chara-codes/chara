...
import { PlusIcon, EditIcon } from "../atoms/icons"
import type { Theme } from "../../styles/theme"
import TechStackDetailView, { type TechStackDetail } from "./tech-stack-detail-view"
import { ButtonBase } from "../atoms/form-elements"
import { useNavigateToAddTechStack, useNavigateToEditTechStack, useNavigateBack } from "../../store/routing-store"
import { useTechStacks } from "../../store/tech-stacks-store"
...
const TechStacksView: React.FC = () => {
  ...
  const [selectedTechStack, setSelectedTechStack] = useState<TechStackDetail | null>(null)
  ...
  if (selectedTechStack) {
    ...
    const updatedTechStack = techStacks.find((stack) => stack.id === selectedTechStack.id) || selectedTechStack
    return <TechStackDetailView techStack={updatedTechStack} onBack={handleBackToList} />
  }
        ...
        {Object.keys(techStacksByCategory).length > 0 ? (
          Object.entries(techStacksByCategory).map(([category, stacks]) => (
            <CategorySection key={category}>
              ...
              <TechStacksGrid>
                {stacks.map((stack) => (
                  <TechStackCard key={stack.id} $isNew={stack.isNew} onClick={() => handleSelectTechStack(stack)}>
                    ...
                    <TechStackDescription>{stack.description}</TechStackDescription>
                    <PopularityBar $level={stack.popularity} />
                  </TechStackCard>
                ))}
              </TechStacksGrid>
            </CategorySection>
          ...
          <NoResultsMessage>
            No tech stacks found for &quot;{searchQuery}&quot;. Try a different search term.
          </NoResultsMessage>
        )}
      </TechStacksContent>
    </TechStacksContainer>
