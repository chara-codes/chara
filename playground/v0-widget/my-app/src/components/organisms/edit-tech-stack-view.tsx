...
import { ArrowLeftIcon, CodeIcon, ServerIcon, DatabaseIcon, GlobeIcon, LayersIcon, PlusIcon } from "../atoms/icons"
import type { TechStackDetail } from "./tech-stack-detail-view"
import { useNavigateBack, useNavigateToTechStacks, useSelectedTechStackId } from "../../store/routing-store"
import { useGetTechStackById, useUpdateTechStack } from "../../store/tech-stacks-store"
import {
  InputBase,
  TextAreaBase,
  SelectBase,
...
const JsonEditor = styled.textarea<{ $hasError?: boolean }>`
  width: 100%;
  padding: ${spacing.md};
  border: 1px solid ${(props) => (props.$hasError ? colors.destructive.default : colors.border.default)};
  border-radius: ${borderRadius.md};
  ...
  &:focus {
    outline: none;
    border-color: ${(props) => (props.$hasError ? colors.destructive.default : colors.primary.default)};
    box-shadow: ${shadows.focus} ${(props) => (props.$hasError ? colors.destructive.light : colors.primary.light)};
  }
`
...
const EditTechStackView: React.FC = () => {
  ...
  const techStack = selectedTechStackId ? getTechStackById(selectedTechStackId) : undefined
  ...
  const validateForm = useCallback(() => {
    ...
    docLinks.forEach((link, index) => {
      if (!link.name && link.url) newErrors[`docLink_${index}_name`] = "Name is required"
      if (link.name && !link.url) newErrors[`docLink_${index}_url`] = "URL is required"
    })
  ...
  const handleSubmit = useCallback(() => {
    if (validateForm()) {
      ...
      const updatedTechStack: TechStackDetail = {
        ...
        documentationLinks: (formData.documentationLinks || []).filter((link) => link.name || link.url),
          ...
          <FormSectionBase>
            ...
            <FormRowBase>
              <FormGroupBase>
                ...
                />
                {errors.name && <ErrorMessageBase>{errors.name}</ErrorMessageBase>}
              </FormGroupBase>
              ...
              <FormGroupBase>
                ...
                {errors.category && <ErrorMessageBase>{errors.category}</ErrorMessageBase>}
              </FormGroupBase>
            </FormRowBase>
            ...
            <FormGroupBase $fullWidth>
              ...
              />
              {errors.description && <ErrorMessageBase>{errors.description}</ErrorMessageBase>}
            </FormGroupBase>

            <FormGroupBase $fullWidth>
              <LabelBase htmlFor="longDescription">Detailed Description</LabelBase>
              <TextAreaBase
                id="longDescription"
          ...
          <FormSectionBase>
            ...
            {(formData.documentationLinks || []).map((link, index) => (
              <LinkItem key={index}>
                <LinkFields>
                  <FormGroupBase $fullWidth>
                    <LabelBase htmlFor={`docLink_${index}_name`}>Name</LabelBase>
                    <InputBase
                    ...
                    {errors[`docLink_${index}_name`] && (
                      <ErrorMessageBase>{errors[`docLink_${index}_name`]}</ErrorMessageBase>
                    )}
                  </FormGroupBase>
                  ...
                  <FormGroupBase $fullWidth>
                    ...
                    {errors[`docLink_${index}_url`] && (
                      <ErrorMessageBase>{errors[`docLink_${index}_url`]}</ErrorMessageBase>
                    )}
                  </FormGroupBase>

                  <FormGroupBase $fullWidth>
                    <LabelBase htmlFor={`docLink_${index}_description`}>Description</LabelBase>
                    <InputBase
            ...
            <AddButton $variant="secondary" $size="small" onClick={handleAddDocLink}>
          ...
          <FormSectionBase>
            ...
            {(formData.mcpServers || []).map((server, index) => (
              <LinkItem key={index}>
                <LinkFields>
                  <FormGroupBase $fullWidth>
                    <LabelBase htmlFor={`mcpServer_${index}_name`}>Server Name</LabelBase>
                    <InputBase
                  ...
                  <FormGroupBase $fullWidth>
                    <LabelBase htmlFor={`mcpServer_${index}_config`}>Configuration (JSON)</LabelBase>
                    <JsonEditor
                    ...
                    />
                    {mcpJsonErrors[index] && <ErrorMessageBase>{mcpJsonErrors[index]}</ErrorMessageBase>}
                  </FormGroupBase>
                </LinkFields>
            ...
            <AddButton $variant="secondary" $size="small" onClick={handleAddMcpServer}>
              <PlusIcon width={14} height={14} />
              Add MCP Server
            </AddButton>
          </FormSectionBase>
