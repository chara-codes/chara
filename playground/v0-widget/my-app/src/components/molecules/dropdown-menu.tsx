...
import { SearchIcon, FileIcon, DocumentIcon, TerminalIcon, UploadIcon, PointerIcon } from "../atoms/icons"
...
const DropdownContainer = styled.div<{ position?: { top: number; left: number } }>`
  position: absolute;
...
const DropdownMenu: React.FC<DropdownMenuProps> = ({ items, isOpen, onClose, position, onSelect }) => {
  ...
  const sortedGroups = Object.entries(groupedItems).sort(([typeA], [typeB]) => typeA.localeCompare(typeB))
        ...
        {sortedGroups.length > 0 ? (
          sortedGroups.map(([type, items]) => (
            <div key={type}>
              ...
              {items.map((item) => (
                <DropdownItemStyled key={item.id} onClick={() => handleItemClick(item)}>
                  {getItemIcon(item)}
                  {item.label}
                </DropdownItemStyled>
              ))}
