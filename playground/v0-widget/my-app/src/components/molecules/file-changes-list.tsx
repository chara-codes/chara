...
const StatusFilter = styled.button<{ $active: boolean; $status?: "kept" | "reverted" | "pending" | "all" }>`
  display: flex;
  align-items: center;
  gap: 4px;
  padding: 4px 8px;
...
const RevertIcon = () => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M7 18L3 14L7 10" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    <path d="M17 10L21 14L17 18" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
)

// Update the FileChangesList component to use status directly from FileDiff
const FileChangesList: React.FC<FileChangesListProps> = ({ diffs, onSelectDiff, onKeepAll, onRevertAll }) => {
  const [statusFilter, setStatusFilter] = useState<"all" | "kept" | "reverted" | "pending">("all")
  ...
  return (
    <Container>
      ...
      <StatusFilterContainer>
        <StatusFilter $active={statusFilter === "all"} $status="all" onClick={() => setStatusFilter("all")}>
          All ({statusCounts.all})
        </StatusFilter>
        {statusCounts.kept > 0 && (
          <StatusFilter $active={statusFilter === "kept"} $status="kept" onClick={() => setStatusFilter("kept")}>
            ...
            return (
              <FileItem
                ...
                onClick={() => {
                  ...
                  setTimeout(() => {
                    const diffElement = document.querySelector(`[data-diff-id="${diff.id}"]`)
                    if (diffElement) {
                      diffElement.scrollIntoView({ behavior: "smooth", block: "start" })
                    } else {
                      console.log(`Could not find diff element with ID: ${diff.id}`)
                    }
                  }, 100)
                }}
              >
                <FileName>
                  ...
                  <StatusBadge $status={diff.status}>
                    {diff.status === "kept" ? "Kept" : diff.status === "reverted" ? "Reverted" : "Pending"}
                  </StatusBadge>
                </FileName>
                <FileStats>
                  {stats.additions > 0 && <StatItem $color="#10b981">+{stats.additions}</StatItem>}
                  {stats.deletions > 0 && <StatItem $color="#ef4444">-{stats.deletions}</StatItem>}
                </FileStats>
              </FileItem>
            )
          })
