...
import { colors, typography, spacing, borderRadius, shadows, transitions } from "../../styles/theme-constants"

// Base input styles
export const InputBase = styled.input<{ $hasError?: boolean; $disabled?: boolean }>`
  width: 100%;
  ...
  background-color: ${(props) => (props.$disabled ? colors.background.tertiary : colors.background.primary)};
  color: ${(props) => (props.$disabled ? colors.text.disabled : colors.text.primary)};
  transition: all 0.2s ease;
  
  &:hover:not(:disabled) {
    border-color: ${(props) => (props.$hasError ? colors.destructive.default : colors.border.hover)};
  }
...
export const IconOptionBase = styled.div<{ $selected: boolean }>`
  ...
  justify-content: center;
  cursor: pointer;
  background-color: ${(props) => (props.$selected ? colors.primary.default : colors.background.tertiary)};
  color: ${(props) => (props.$selected ? colors.background.primary : colors.text.tertiary)};
  border: 1px solid ${(props) => (props.$selected ? colors.primary.default : colors.border.default)};
  transition: all ${transitions.normal} ease;
  
  &:hover {
    background-color: ${(props) => (props.$selected ? colors.primary.default : colors.border.default)};
  }
`
