...
export const useModelsStore = create<ModelsState>()((set) => ({
  ...
  initializeStore: async () => {
    ...
    try {
      const { models, recentModels } = await fetchModels()
      set({
        models: models.length > 0 ? models : fallbackModels,
        recentModels: recentModels.length > 0 ? recentModels : ["claude-3.7-sonnet"],
        isLoading: false,
      })
    } catch (error) {
      ...
      set({
        ...
        isLoading: false,
        loadError: error instanceof Error ? error.message : "Failed to load models data",
      })
    }
  },
