 ...
 */
export const useElementSelector = (onAddContext: (contextItem: any) => void): UseElementSelectorReturn => {
   ...
   */
  const detectComponentInfo = useCallback((element: HTMLElement): ComponentInfo => {
    ...
    try {
      ...
      if (componentInfo.componentName !== "Unknown" && !componentInfo.componentPath) {
        // Convert PascalCase to kebab-case for the file path
        const kebabCase = componentInfo.componentName.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase()
    ...
    while (fiber && !foundName && depth < maxDepth) {
      if (fiber.type) {
        ...
        if (typeof fiber.type === "function") {
          componentInfo.componentName = fiber.type.displayName || fiber.type.name || "UnnamedComponent"
          foundName = true
        }
        // Check for forwardRef and memo components
          ...
          if (fiber.type.render && typeof fiber.type.render === "function") {
            componentInfo.componentName = fiber.type.render.name || fiber.type.displayName || "ForwardRefComponent"
            foundName = true
          }
          // Memo components
          else if (fiber.type.$$typeof && fiber.type.type && typeof fiber.type.type === "function") {
            componentInfo.componentName = fiber.type.type.displayName || fiber.type.type.name || "MemoComponent"
            foundName = true
          }
      ...
      if (!foundName && fiber.stateNode && fiber.stateNode.constructor) {
        if (fiber.stateNode.constructor.name && fiber.stateNode.constructor.name !== "HTMLDivElement") {
   ...
   */
  const extractComponentInfoFromDataAttributes = (element: HTMLElement, componentInfo: ComponentInfo): void => {
    ...
    if (dataComponentName) {
      ...
      if (testIdParts.length > 1) {
        ...
        componentInfo.componentName = testIdParts.map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join("")
      } else {
   ...
   */
  const extractComponentInfoFromClassNames = (element: HTMLElement, componentInfo: ComponentInfo): void => {
    ...
    for (const className of classNames) {
      ...
      if (moduleMatch && moduleMatch[1]) {
        ...
        break
      } else if (styledMatch && styledMatch[1] && componentInfo.componentName === "Unknown") {
        ...
        if (componentInfo.componentName.length > 0) {
          ...
          componentInfo.componentName =
            componentInfo.componentName.charAt(0).toUpperCase() + componentInfo.componentName.slice(1)
   ...
   */
  const extractComponentInfoFromId = (element: HTMLElement, componentInfo: ComponentInfo): void => {
    // Check for PascalCase or camelCase IDs that might indicate component names
    const idMatch = element.id.match(/([A-Z][a-zA-Z0-9]+)(Container|Wrapper|Component|Root|Inner)?$/)
    if (idMatch) {
   ...
   */
  const extractComponentInfoFromParents = (element: HTMLElement, componentInfo: ComponentInfo): void => {
    // Check for common component patterns in parent elements
    let parent = element.parentElement
    let depth = 0
    const maxDepth = 5 // Limit depth to prevent performance issues

    while (parent && depth < maxDepth && componentInfo.componentName === "Unknown") {
      ...
      if (parent.id && parent.id.includes("-") && parent.id.length > 3) {
        ...
        if (idParts.length > 1) {
          componentInfo.componentName = idParts.map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join("")
        }
      }
      ...
      if (role && componentInfo.componentName === "Unknown") {
        componentInfo.componentName = role.charAt(0).toUpperCase() + role.slice(1)
      }
  ...
  const cleanupElementSelectionUI = useCallback(() => {
    ...
    if (cleanupRef.current.handleMouseMove) {
      document.removeEventListener("mousemove", cleanupRef.current.handleMouseMove)
    }
  }, [])
  ...
  const endElementSelection = useCallback(() => {
    ...
    const chatTrigger = document.querySelector('[aria-label="Open chat panel"], [aria-label="Close chat panel"]')
    ...
    const styleElement = document.querySelector("style[data-element-selection]")
    ...
    if (cleanupRef.current) {
      if (cleanupRef.current.handleElementClick) {
        document.removeEventListener("click", cleanupRef.current.handleElementClick, true)
      }
      if (cleanupRef.current.handleKeyDownEscape) {
        document.removeEventListener("keydown", cleanupRef.current.handleKeyDownEscape)
      }
      if (cleanupRef.current.handleMouseMove) {
        document.removeEventListener("mousemove", cleanupRef.current.handleMouseMove)
      }
  ...
  /**
   * Creates a comment modal for the selected element
   * @param element The DOM element to create a comment modal for
   * @returns The confirm button element from the modal
   */
  const createCommentModal = useCallback(
    ...
    [cleanupElementSelectionUI, detectComponentInfo, elementComment, endElementSelection, onAddContext],
  )

  /**
   * Creates the modal DOM elements
   */
  const createModalElements = (element: HTMLElement, componentInfo: ComponentInfo) => {
  ...
  const createElementInfoSection = (element: HTMLElement) => {
    ...
    const classText =
      element.className && typeof element.className === "string" && element.className.trim()
  ...
  const createComponentInfoSection = (componentInfo: ComponentInfo) => {
    ...
    componentBadge.textContent = componentInfo.isReactComponent ? "React Component" : "Component"
  ...
  /**
   * Starts element selection mode, allowing the user to click on elements
   * to select them and add comments.
   */
  ...
  }, [createCommentModal, detectComponentInfo, endElementSelection])
  ...
  const createSelectionGuide = (config: ElementSelectorUIConfig) => {
    ...
    Object.assign(selectionGuide.style, {
      ...
      borderRadius: "8px",
      zIndex: `${config.zIndexBase + 5}`,
      boxShadow: "0 4px 20px rgba(0, 0, 0, 0.5), 0 0 0 3px rgba(255, 255, 255, 0.8)",
      fontWeight: "600",
      fontSize: "18px",
      display: "flex",
      alignItems: "center",
    ...
    textSpan.textContent = "Click on any element to select it. Press ESC to cancel."
  ...
  const setupSelectionEventHandlers = () => {
    ...
    console.log("DEBUG: Event listeners attached:", cleanupRef.current ? Object.keys(cleanupRef.current) : "None")
  }
  ...
  const updateTagDisplay = (element: HTMLElement, e: MouseEvent) => {
    ...
    if (tagDisplay) {
      ...
      const componentInfo = detectComponentInfo(element)
      const componentText =
        componentInfo.isReactComponent && componentInfo.componentName !== "Unknown"
          ? ` (${componentInfo.componentName})`
          : ""

      tagDisplay.textContent = tagName + idText + (idText ? "" : classText) + componentText
