...
export const subscription = publicProcedure.subscription(async function* (
  opts
) {
  ...
  try {
    ...
    while (!abortController.signal.aborted) {
      ...
      const result = await Promise.race([
        eventListeners.ping
          .next()
          .then((value) => ({ type: "server_ping", value })),
      ...
      switch (result.type) {
        ...
        case "instructions_summary":
          console.log(
            `[SUBSCRIPTION] Summary stream event received (ID: ${data.summaryId})`
          );
...
async function* handleSummaryStream(
  ...
  console.log(
    `[SUBSCRIPTION] Sent instructions_summary_start event for summary ${data.summaryId}`
  );
  ...
  try {
    ...
    for await (const chunk of data.stream) {
      ...
      console.log(
        `[SUBSCRIPTION] Streaming summary chunk ${chunkCount}: "${chunk.substring(
          0,
          30
        )}${chunk.length > 30 ? "..." : ""}"`
      );
    ...
    console.log(
      `[SUBSCRIPTION] Sent instructions_summary_end event for summary ${data.summaryId}`
    );
  } catch (error) {
    ...
    console.log(
      `[SUBSCRIPTION] Sent instructions_summary_error event for summary ${data.summaryId}`
    );
  }
}
