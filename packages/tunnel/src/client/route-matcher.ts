...
export class RouteMatcher {
  ...
  public addRoute(route: RouteOptions): void {
    ...
    if (!route.handler && !route.redirect) {
      logger.error(
        `Route ${route.method || "ALL"} ${
          route.url
        } is invalid: must provide either handler or redirect configuration`
      );
      throw new Error(
        "Route must have either a handler or redirect configuration"
      );
    }
    ...
    const hasWildcardParams =
      normalizedUrl.includes(":") && normalizedUrl.includes("*");

    logger.debug(
      `Registering route: ${
        routeConfig.method === "ALL" ? "ALL METHODS" : routeConfig.method
      } ${routeConfig.url}${
        routeConfig.url.includes(":")
          ? ` (with ${hasOptionalParams ? "optional " : ""}${
              hasWildcardParams ? "wildcard " : ""
            }parameters)`
          : ""
  ...
  public matchPath(
    ...
    const requiredSegments = patternSegments.filter(
      (segment) =>
        !segment ||
        (!this.isOptionalParam(segment) && !this.isWildcardParam(segment))
    ).length;
    ...
    const hasWildcard = patternSegments.some(
      (segment) => segment && this.isWildcardParam(segment)
    );
