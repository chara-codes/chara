...
import { EllipsisVertical, ExternalLink, Code, Layers, Info, X } from "lucide-react";
    ...
    <DropdownMenu>
      ...
      <DropdownMenuPortal>
        <DropdownMenuContent side="top" align="end" sideOffset={10}>
          <div className="px-2 py-1 text-md font-semibold">
            Actions
          </div>
          <DropdownMenuItem>
            Edit stack
...
const CardChipWithLink = ({ label, link, codeLink }: { label: string; link: string; codeLink: string; }) => {
  return (
...
const StackCard = ({ stackName, technologies, category, description }: {
  stackName: string;
    ...
    <CardContent>
      ...
      <div className="flex flex-wrap gap-x-2 gap-y-3 pr-8">
        {technologies.map(({ name, docLink, codeLink}) => (
          <CardChipWithLink key={name} label={name} link={docLink} codeLink={codeLink} />
        ))}
      </div>
    </CardContent>
        ...
        <DialogContent className="w-full max-w-xl">
          ...
          <section className="mb-4">
            ...
            <ReactMarkdown components={{
              ...
              ul: ({ node, ...props }) => (
                <ul className="list-disc pl-5 space-y-2 marker:text-border" {...props} />
              ),
              ol: ({ node, ...props }) => (
