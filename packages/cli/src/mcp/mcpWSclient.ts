...
class MCPClient {
  ...
  public listTools = async (command: string, params: string): Promise<Tool[]> => {
    const allTools: Tool[] = [];
...
export const initMCPClient = async (): Promise<void> => {
  ...
  trpc.mcpClientsSubscriptions.subscribe({ clientId: CLIENT_ID }, {
    onData: async (instruction) => {
      ...
      switch (instruction.command) {
        case 'tools/list':
          const allTools: Tool[] = await clients.listTools(instruction.command, instruction.params);
          response = JSON.stringify(allTools);
          break;

        case 'tools/call':
          const toolRes = await clients.getTool(instruction.command, instruction.params);
          response = JSON.stringify(toolRes);
          break;

        case 'prompts/list':
          const allPrompts = await clients.listPrompts(instruction.command, instruction.params);
          response = JSON.stringify(allPrompts);
          break;

        case 'prompts/get':
          const promptRes = await clients.getPrompt(instruction.command, instruction.params);
          response = JSON.stringify(promptRes);
          break;

        case 'resources/list':
          const allResources = await clients.listResources(instruction.command, instruction.params);
          response = JSON.stringify(allResources);
          break;

        case 'resources/read':
          const resourceObj = await clients.getResource(instruction.command, instruction.params);
          response = JSON.stringify(resourceObj);
          break;

        case 'resources/templates/list':
          const allTemplates = await clients.listTemplates(instruction.command, instruction.params);
