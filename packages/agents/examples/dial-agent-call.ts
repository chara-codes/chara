...
async function dialAgentExample() {
  ...
  try {
    ...
    const thinkingModel = dialModels.find(m => m.id.includes("with-thinking"));
    if (thinkingModel) {
...
async function runBasicExample(modelId: string) {
  ...
  const messages: CoreMessage[] = [
    { role: "user", content: "Hello, what can you tell me about quantum computing?" },
  ];
...
async function runThinkingExample(modelId: string) {
  ...
  const messages: CoreMessage[] = [
    { 
      role: "user", 
      content: "I need to solve this complex problem: If a recipe calls for 2.5 cups of flour to make 12 cookies, how much flour is needed to make 30 cookies? Please show your reasoning step by step." 
    },
  ];
...
export { dialAgentExample, runBasicExample, runConversationExample, runThinkingExample };