...
async function demonstrateModelFetching() {
  ...
  try {
    ...
    const providersWithModels = Object.entries(allModels).filter(([_, models]) => models.length > 0).length;
    
    logger.info('📊 Statistics:', {
      totalProviders: Object.keys(allModels).length,
      providersWithModels,
      totalModels,
      averageModelsPerProvider: Math.round(totalModels / Object.keys(allModels).length)
    });
  ...
  try {
    ...
    const latestModels = Object.entries(allModels)
      .flatMap(([provider, models]) => 
        models
          .filter(m => m.id.toLowerCase().includes('latest') || m.id.toLowerCase().includes('new'))
          .map(m => ({ provider, model: m.id }))
      );
...
async function demonstrateErrorHandling() {
  ...
  const availableProviders = getAvailableProviders();
  const providerWithoutFetching = availableProviders.find(p => !p.fetchModels);
  ...
  if (providerWithoutFetching) {
    try {
      ...
      logger.success(`Correctly caught error for ${providerWithoutFetching.name} (no model fetching support):`, {
        error: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  }
