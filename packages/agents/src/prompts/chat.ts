...
export const chatPrompt = (options: ChatPromptOptions = {}) => {
  ...
  return `You are a highly skilled software engineer with extensive knowledge in many programming languages, frameworks, design patterns, and best practices.
...
${hasTool("grep") ? `- When looking for symbols in the project, prefer the \`grep\` tool.
...
${hasTool("examination") ? `## Code Quality and Diagnostics
...
${hasTool("terminal") ? `## Terminal Commands Execution
