...
/**
 * Development Server Control Tool
 *
 * A comprehensive diagnostic and monitoring tool for managing and testing running development servers.
 *
 * IMPORTANT: Agents MUST check the status of development servers using this tool every time
 * before attempting to run or interact with dev servers. This ensures proper server state
 * management and prevents conflicts.
 *
 ...
 *
 * Usage Examples:
 * - devServer.execute({}) // List all processes - USE THIS FIRST ALWAYS
 * - devServer.execute({ processId: "server-id" }) // Test specific process
 * - devServer.execute({ processId: "api", endpoint: "/health", method: "POST" }) // Custom endpoint test
 */
export const devServer = tool({
  description: `Development server control and diagnostic tool for managing running dev servers.

  CRITICAL: Agents MUST use this tool to check server status EVERY TIME when need to communicate with dev server, don't use terminal tool for development server.

  This tool provides two main functionalities:

  1. PROCESS DISCOVERY (when no processId provided) - ALWAYS RUN THIS FIRST:
     - Lists all currently running development server processes
     ...
     - REQUIRED before starting new servers or making server requests
    ...
    try {
      ...
      if (!processId) {
        ...
        allProcesses.forEach((process, index) => {
          ...
          result += `   Uptime: ${
            info.uptime ? Math.round(info.uptime / 1000) : "unknown"
          } seconds\n`;
          result += `   Start Time: ${
            info.startTime ? info.startTime.toISOString() : "N/A"
          }\n`;

          // Add logs for each process
          const processLogs = runnerService.getProcessLogs(id);
          if (processLogs.length > 0) {
            result += `   Recent Logs (last ${Math.min(
              processLogs.length,
              10
            )} entries):\n`;
        ...
        result += `devServer({ processId: "process-id" })`;
      ...
      // Step 2: Get current logs count (before HTTP call)
      const logsBeforeCount = runnerService.getProcessLogs(processId).length;

      // Step 3: Make HTTP call to the server
      const url = `${serverInfo.serverUrl}${
      ...
      // Format the result
      let result = `Dev Server Diagnostic for Process: ${processId}\n`;
      result += `Server: ${serverInfo.name} (${serverInfo.status})\n`;
      result += `URL: ${serverInfo.serverUrl}\n`;
      result += `Uptime: ${
        serverInfo.uptime ? Math.round(serverInfo.uptime / 1000) : "unknown"
      } seconds\n\n`;
