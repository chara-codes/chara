  ...
  useEffect(() => {
    if (isThinking && isThinkingExpanded && thinkingContentRef.current) {
      const scrollToBottom = () => {
        ...
        if (onTerminalStreamData) {
          onTerminalStreamData(data);
        }
      },
      [onTerminalStreamData],
    );

    // Expose terminal streaming handler via ref
    useImperativeHandle(
      ref,
      () => ({
        handleTerminalStreamData,
      }),
      [handleTerminalStreamData],
    );
    const [activeTab, setActiveTab] = useState<"commands" | "diffs">("diffs");
          ...
          {showDeleteConfirm && (
            ...
            >
              <p style={{ margin: "0 0 8px 0", fontSize: "12px" }}>
                Delete this message and all subsequent messages? All changes
                will roll back.
              </p>
              <div
                style={{
