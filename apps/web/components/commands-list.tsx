...
export function CommandsList({ commands }: CommandsListProps) {
  ...
  const [commandResults, setCommandResults] = useState<
    Record<string, { success: boolean; output: string }>
  >({});
  const [isRunningAll, setIsRunningAll] = useState(false);
  const [currentCommandIndex, setCurrentCommandIndex] = useState<number | null>(
    null,
  );
      ...
      <div className="bg-gray-900 text-gray-200 p-1">
        {commands.map((cmd, index) => (
          <div key={index} className="relative group">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        (isRunningAll && currentCommandIndex === index) ? (
            ...
            {cmd.description && (
              <div className="text-xs text-gray-400 pl-2 pb-2">
                {cmd.description}
              </div>
            )}
